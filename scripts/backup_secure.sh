...
mysql_pwd_escaped=$(printf '%q' "$DB_PASSWORD")
ssh "$WP_SSH_USER@$WP_SSH_HOST" "export MYSQL_PWD=$mysql_pwd_escaped; mysqldump --single-transaction --quick --lock-tables=false -u '$DB_USER' -h '$DB_HOST' '$DB_NAME'" | gzip > "$DB_DUMP_FILE"
